...
class ALU_agent extends uvm_agent;
	...
	ALU_sequencer		sa_seqr;
	ALU_driver		sa_drvr;
	ALU_monitor_before	sa_mon_before;
	ALU_monitor_after	sa_mon_after;
...
// Where the UVM component creates its resources
	...
	// //Correct UVM way to create sequencers, drivers, monitors, agents, envs
		sa_seqr		= ALU_sequencer::type_id::create(.name("sa_seqr"), .parent(this));
		sa_drvr		= ALU_driver::type_id::create(.name("sa_drvr"), .parent(this));
		sa_mon_before	= ALU_monitor_before::type_id::create(.name("sa_mon_before"), .parent(this));
		sa_mon_after	= ALU_monitor_after::type_id::create(.name("sa_mon_after"), .parent(this));
