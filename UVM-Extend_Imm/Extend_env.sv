...
// UVM Environment for Extend

import uvm_pkg::*;
`include "uvm_macros.svh"

class Extend_env extends uvm_env;
//This module just generates stimulus and connects things
	`uvm_component_utils(Extend_env) // UVM macro to register the class with the factory
	//These are pointers
	Extend_agent sa_agent;
	Extend_scoreboard sa_sb;
	...
	function void build_phase(uvm_phase phase);
		...
		sa_agent	= Extend_agent::type_id::create(.name("sa_agent"), .parent(this));
		sa_sb		= Extend_scoreboard::type_id::create(.name("sa_sb"), .parent(this));
