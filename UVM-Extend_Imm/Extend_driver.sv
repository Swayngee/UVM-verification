// Drake Gonzales
// drgonzales@g.hmc.edu
// UVM Driver for Extend

import uvm_pkg::*;
`include "uvm_macros.svh"

class Extend_driver extends uvm_driver#(Extend_transaction); 
// The # used above is an SV parameter and it represents the data type used in the sequencer
	`uvm_component_utils(Extend_driver) // Utils macro

	virtual Extend_if vif; // virtual interface handle

	function new(string name, uvm_component parent); // Class constr again
		super.new(name, parent);
	endfunction: new

	function void build_phase(uvm_phase phase); // Starts the build phase of the class. Is executed before the run phase
		super.build_phase(phase);
		void'(
			uvm_resource_db#(virtual Extend_if)::read_by_name(
			.scope("*"), 
			.name("Extend_if"), 
			.val(vif)
			)
		);
	endfunction: build_phase

	task run_phase(uvm_phase phase); // Run phase where the code of the driver is executed
		drive();
	endtask: run_phase

	virtual task drive(); //Just a combinational task to drive the DUT inputs and capture outputs
		Extend_transaction tr;
        forever begin
            // Get next transaction from sequencer
            seq_item_port.get_next_item(tr);

            // Drive inputs to DUT
            vif.sig_Instr    <= tr.Instr;
            vif.sig_ImmSrc   <= tr.ImmSrc;


            // Capture DUT output 
            tr.ImmExt = vif.ImmExt;

            // Tell sequencer we are done with this item
            seq_item_port.item_done();
 	    #1;
        end
	endtask: drive
endclass: Extend_driver
